...
def make_sma_delta(num_candles, candle_period: PeriodEnum, sma_period):
    def feature_sma_delta(market_info: BacktestingMarketInfo, pair):
        current_sma = get_indicator(market_info, pair,
                                    IndicatorEnum.SMA, [sma_period], 0, candle_period)[0]
        for i in range(num_candles):
            sma = get_indicator(market_info, pair,
                                IndicatorEnum.SMA, [sma_period], i, candle_period)[0]
            yield Feature(name='sma_delta_sma_period_{}_candle_period_{}_offset_{}'
                          .format(sma_period, candle_period.name, i),
                          value=(current_sma - sma) / current_sma)
    return feature_sma_delta
