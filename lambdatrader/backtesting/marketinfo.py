...
class BacktestMarketInfo:
    ...
    def get_pair_last_24h_btc_volume(self, pair):
        if pair in self.__last_volume_calc_timestamp:
            if self.__last_volume_calc_timestamp[pair] == self.get_market_time():
                return self.__last_volume_calc_volume[pair]
            elif self.__last_volume_calc_timestamp[pair] == self.get_market_time() - 300:
                ...
                self.__last_volume_calc_timestamp[pair] = self.get_market_time()
    ...
    def get_pair_last_24h_high_cached(self, pair):
        if pair in self.__last_high_calc_timestamp:
            if self.__last_high_calc_timestamp[pair] == self.get_market_time():
                return self.__last_high_calc_high[pair]
            elif self.__last_high_calc_timestamp[pair] == self.get_market_time() - 300:
                high = self.__last_high_calc_high[pair]
                try:
                    high_omitted = self.get_pair_candlestick(pair=pair, ind=24 * 12).high
                    if high_omitted == high:
                        del self.__last_high_calc_timestamp[pair]
                ...
                self.__last_high_calc_timestamp[pair] = self.get_market_time()
